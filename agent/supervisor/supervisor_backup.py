...
def run_task(task_or_path):
    ...
    Accepts either a YAML file path or a TaskDefinition instance.
    """
    if isinstance(task_or_path, TaskDefinition):
        task = task_or_path
        yaml_path = None
    else:
        yaml_path = Path(task_or_path)
        task = load_task_from_yaml(str(yaml_path))
    ...
    if yaml_path and yaml_path.is_file():
        original_yaml_path.write_text(yaml_path.read_text(encoding="utf-8"), encoding="utf-8")
    else:
        # Serialize TaskDefinition to JSON-safe primitives, then dump as YAML
        task_json = json.loads(task.json())
        original_yaml_path.write_text(yaml.safe_dump(task_json), encoding="utf-8")
    ...
    for step in steps:
        ...
        while True:
            ...
            if attempts >= step.stop_rules.max_attempts:
                # Try self-healing before escalating
                if SELF_HEALING_AVAILABLE and healing_attempts < max_healing_attempts:
                    ...
                    if corrected_yaml:
                        healing_attempts += 1
                        log_healing_attempt(Path(run_path), healing_attempts, True, {"corrected_yaml": corrected_yaml})
                        print(f"[SELF-HEALING] Generated corrected plan, retrying...")
                        # Reset attempts and retry with corrected plan
                        attempts = 0
                        # In production, would reload the corrected YAML
                        continue
                    else:
                        log_healing_attempt(Path(run_path), healing_attempts + 1, False, {"reason": "no_fix_generated"})
            ...
            # Execute
            # Allow primary tool by default; enforce allowlist only when provided
            allowed_tools = step.tools_allowed or task.tools_allowed or [step.type.value]
            if step.type.value not in allowed_tools:
