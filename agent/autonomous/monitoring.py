...
class ResourceMonitor:
    ...
    def __init__(self, memory_limit_mb: float = 1024):
        ...
        Args:
            memory_limit_mb: Maximum allowed memory in MB (default: 1024)
        """
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        if metrics.memory_mb > self.memory_limit_mb:
            ...
            health["warnings"].append(
                f"Memory usage {metrics.memory_mb:.1f}MB exceeds limit {self.memory_limit_mb}MB"
            )

        if metrics.open_files > 100:
            health["warnings"].append(
                f"High number of open files: {metrics.open_files}"
            )

        if metrics.threads > 50:
            health["warnings"].append(
                f"High number of threads: {metrics.threads}"
            )
